...
def main():
    ...
    for text in chunks:
        ...
        if calibration_mode:
            ...
            # 1. pre-check phase
            # this phase will identify the pairs of block and step having high redundancy by
            # computing the similarity of the attention map with a diagonal matrix
            # and they will be applied the compression methods earlier than the others
            pre_hooks = pre_calibration_check(ema_model)
            ...
            threshold = threshold_q(similarities, ratio=0.1) # decide threshold with percentile
            if nopre:
            ...
            calibration_reset(ema_model.transformer) # reset is important
            
            # 2. pre-calibration phase
            # in this phase, we try to apply TS to those pairs we've identified in the pre-check phase
            pre_calibrate_hook = pre_calibration(ema_model, steps=nfe_step, threshold=delta)
            ...
            # 3. calibration phase
            # this is final calibration, for all blocks and steps we will apply both of TS and BS while maintaining
            # the output quality within threshold we set
            calibrate_hook = calibration(ema_model, steps=nfe_step, threshold=delta)
