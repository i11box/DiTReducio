...
from tts.calibration.util_calibration import threshold_q, seed_everything
        ...
        with torch.inference_mode():
            ...
            for seg_i, text in enumerate(text_segs):
                ...
                with torch.cuda.amp.autocast(dtype=self.precision, enabled=True):
                    ...
                    if not hasattr(self.dit, 'infer_times'):
                        self.dit.infer_times = []
                    self.dit.infer_times.append(infer_time)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--calibration_mode', '-q', type=bool, default=False, help='Calibration Mode')
